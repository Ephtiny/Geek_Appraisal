...
答：n为偶数时，从最开始的n个叶结点组成n/2个间接结点，再由这n/2个间接结点组成下一组（n/2)/2个间接结点，直到组成最后的根结点，这么加起来一共有2*n-1个结点；n为奇数时，从最开始的n个结点每取两个最小的结点组成一个间接结点并去除相加的两个结点，最终总共出现2*n-1个结点

2. 该代码可以对大写字母的字符串进行huffman编码，如果要读小写字母字符串进行编码，应该如何修改？如果想要对同时具有大写和小写字母的字符串都进行编码呢（写出思路即可）？

答：将31行的0x41改为0x61，并将181行中的'A'改为'a';arr_big[27]中的27要改为54，同时27到41行的遍历需将31行的0x41改为0x61，并再遍历一次，此次遍历中的i要从28开始，181行的打印要先分辨是大写字母还是小写字母，然后再进行打印
