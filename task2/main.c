...
int main()
{
    ...
    for(int i = 0; i < 27; i++)
    {
        ...
        {
            arr_small[count2++] = arr_big[i];	//将arr_big中不等于0的值（即出现过的字母的次数）赋给arr_small	/*补充注释（说明作用）*/
        }
    }
    ...
    {
        int length = 0;
        if(huffman[i][1] == -1 && huffman[i][2] == -1)  //对所有没有左右子的结点即叶节点进行下列操作	/*补充判断模块注释*/
        {
            length = findParent(i,huffman,count);   //得到各个叶结点的带权路径长度
            sum+=length*huffman[i][3];/*自行填充代码*/  	//各个叶结点带权路径长乘以权重并度累加得到总长度WPL
        }
    }
...
void huffmanTree(int w[],int huffman[][4],int n)
{
    ...
    //将初始权重数组arr_small中的各个值存入huffman的前n项	/*补充注释（说明作用）*/
    for(int i = 0; i < n; i++)
    {
        huffman[i][3] = w[i];
    }
...
void findMin(int ii[],int huffman[][4],int n)
{
    ...
    for(int i = 0; i < 2 * n - 1; i++)
    {
        if(huffman[i][3] == -1 && huffman[i][0] == -1) 	//发现有未赋值的结点则退出	 /*补充判断模块注释（说明作用）*/
        {
            break;
        }
        if(huffman[i][3] < min && huffman[i][0] == -1) 	//将还没有父结点的结点中小于min的结点的值赋予min /*补充判断模块注释（说明作用）*/
        {
            min = huffman[i][3];
            ii[0] = i;
        }
    ...
    for(int i=0;i<2*n-1;i++)
	{
		if(huffman[i][3]<min2&&huffman[i][3]>min&&huffman[i][0]==-1)
		{
			min2=huffman[i][3];
			ii[1]=i;
		}
...
{
    int length = 0;
    if(huffman[i][0] == -1)     //该函数进行到结点找到最后的父结点即根结点	/*补充判断模块注释*/
    {
        return 0;
    }
...
void HuffmanCode(int i,int huffman[][4],int n)
{
    ...
    while(father != -1)
    {
        if(current==huffman[father][1])	/*自行填充代码*/	//判断当前结点的父节点左子树是否为当前结点
			code[start]=0;	/*自行填充代码*/	//子结点是父节点的左子树，编码为0
		else
			code[start]=1;	/*自行填充代码*/	//子结点是父节点的右子树，编码为1
		
		current=father;	/*自行填充代码*/	//往上溯源，更新当前结点
		father=huffman[father][0];	/*自行填充代码*/	//同理（当前结点更新后），更新当前结点的父节点
		start++;	/*自行填充代码*/	//更新编码位置
    }
    code[start]='\0';   //编码结束符
    ...
    //printf("%c Huffman code:  %d\n",'A'+i,code);    //打印字符的huffman编码
    printf("%c Huffman code:  ",'A'+i); 
